...
ms_codes <- eurocontrol::member_state %>%
  ...
  filter(!icao %in% c("BK", "ET", "EL", "LN")) %>%
  pull(icao) %>%
  append("GC") |>
  sort() %>%
  unique()

til_latest <- ymd("2026-01-01") # excluded
wef_latest <- til_latest - months(12)

wef_reference <- wef_latest |> `year<-`(2019)
til_reference <- til_latest |> `year<-`(2020)
